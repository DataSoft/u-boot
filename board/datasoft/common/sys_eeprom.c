...
static struct __attribute__ ((__packed__)) eeprom {
	...
	u32 crc;          /* (x + 1) - (x + 5) CRC32 checksum */
#endif
} e;
...
static int prog_eeprom(void)
{
	...
	/*
	 * The AT24C256 datasheet says that data can be written in page
	 * mode, which means 64 bytes at a time, and it takes up to 5ms to
	 * complete a given write.
	 */
	for (i = 0, p = &e; i < sizeof(e); i += 64, p += 64) {
		ret = i2c_write(CONFIG_SYS_I2C_EEPROM_ADDR, i, CONFIG_SYS_I2C_EEPROM_ADDR_LEN,
				p, min((int)(sizeof(e) - i), 64));
		if (ret)
			break;
...
static void set_mac_address(unsigned int index, const char *string)
{
	...
	eth_parse_enetaddr(string, e.mac[index]);
	update_crc();
}
...
int mac_read_from_eeprom(void)
{
	...
	for (i = 0; i < min(e.mac_count, (u8)MAX_NUM_PORTS); i++) {
		if (memcmp(&e.mac[i], "\0\0\0\0\0\0", 6) &&
			...
			eth_setenv_enetaddr(enetvar, e.mac[i]);
		}
	}
